...
def write_sector(p: dict, chunk_paths: list[Path], data: dict) -> None:
    ...
    for i, pt in enumerate(data.get("lights", [])):
        ...
        lines.append(f"transform = Transform3D(1, 0, 0, 0, 1, 0, 0, 0, 1, {x}, {y}, {z})\n")
        lines.append("light_color = Color(0.96, 0.95, 0.84, 1)\n")
        lines.append("light_energy = 1.4\n")
        lines.append("light_specular = 0.14\n")
        lines.append("shadow_enabled = false\n")
        lines.append("omni_range = 8.5\n")
...
def write_main() -> None:
    ...
    lines.append("ambient_light_color = Color(0.43, 0.42, 0.36, 1)\n")
    lines.append("ambient_light_energy = 0.24\n")
    lines.append("tonemap_mode = 2\n")
    lines.append("tonemap_exposure = 1.08\n")
    ...
    lines.append("glow_intensity = 0.2\n")
    lines.append("fog_enabled = true\n")
    lines.append("fog_light_color = Color(0.43, 0.42, 0.36, 1)\n")
